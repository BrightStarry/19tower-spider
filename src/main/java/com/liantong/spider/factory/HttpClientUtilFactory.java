...
/**
 * {@link com.liantong.spider.util.HttpClientUtil} 工厂
 */
@Component
public class HttpClientUtilFactory {

    private final HttpClientUtil.DefaultHttpClientConfig httpClientConfig;

    /**
     * 构造时创建默认的 httpClient设置
     */
    public HttpClientUtilFactory(SpiderConfig config) {
        httpClientConfig = config.getHttpClient();
     ...
     * 构建新的 HttpClientUtil
     */
    public HttpClientUtil buildHttpClientUtil(HttpClientUtil.HttpClientConfigurable config) {
        return new HttpClientUtil(config);
    }

    /**
     * 构建默认的 HttpClientUtil
     * 使用默认的{@link this#httpClientConfig}
     */
    public HttpClientUtil buildDefaultHttpClientUtil() {
        return buildHttpClientUtil(httpClientConfig);
    }
}
