...
public class HtmlResolver {
    ...
    /**
     * 抽取html中指定元素, 当返回的 {@link Elements#size() == 0 }时,表示没有该元素
     */
    public static Elements getElements(String html, String selector) {
        return getDocument(html).select(selector);
    }

    /**
     * 抽取html中指定元素, 当返回的 {@link Elements#size() == 0 }时,表示没有该元素
     */
    public static Elements getElements(Element document, String selector) {
